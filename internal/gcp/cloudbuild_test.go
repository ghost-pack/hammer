...
func TestCreateOrUpdateCloudBuildTrigger(t *testing.T) {
	t.Run("fails to parse cloud build yaml", func(t *testing.T) {
		...
		client := newCloudBuildClientWithAPI(mockAPI)
		err := client.CreateOrUpdateCloudBuildTrigger(context.Background(), "my-project", "12345", "us-central1", "testdata/cloudbuild_bad_yaml.yaml", "webhook", "my_trigger", "", "sa-pipeline@hammer-central-prod.iam.gserviceaccount.com", false)
	...
	t.Run("fails to create trigger due to bad trigger type", func(t *testing.T) {
		...
		client := newCloudBuildClientWithAPI(mockAPI)
		err := client.CreateOrUpdateCloudBuildTrigger(context.Background(), "my-project", "12345", "us-central1", "testdata/cloudbuild.yaml", "asdfoydfs", "my_trigger", "", "sa-pipeline@hammer-central-prod.iam.gserviceaccount.com", false)
	...
	t.Run("fails to find trigger", func(t *testing.T) {
		...
		client := newCloudBuildClientWithAPI(mockAPI)
		err := client.CreateOrUpdateCloudBuildTrigger(context.Background(), "my-project", "12345", "us-central1", "testdata/cloudbuild.yaml", "webhook", "my_trigger", "", "sa-pipeline@hammer-central-prod.iam.gserviceaccount.com", false)
...
func Test_createBuildTrigger(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, _ := createBuildTrigger(tt.args.projectID, tt.args.projectNumber, tt.args.triggerName, "webhook", "", "sa-pipeline@hammer-central-prod.iam.gserviceaccount.com", false, tt.args.cfg)
			require.Equal(t, tt.want, got)
		})
	}
}
